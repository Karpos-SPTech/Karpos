            ...
            .then(
                function (resultadoAutenticar) {
                    ...
                    if (resultadoAutenticar.length == 1) {
                        ...
                        fazendaModel.buscarfazendaPorEmpresa(resultadoAutenticar[0].empresaId)
                            .then((resultadoFazenda) => {
                                if (resultadoFazenda.length > 0) {
                                    res.json({
                                        id: resultadoAutenticar[0].id,
                                        email: resultadoAutenticar[0].email,
                                        nome: resultadoAutenticar[0].nome,
                                        senha: resultadoAutenticar[0].senha,
                                        fazenda: resultadoFazenda[0].fazenda
                                    });
                                } else {
                                    res.status(204).json({ fazenda: [] });
                                }
                            })
...
function cadastrar(req, res) {
    ...
    if (nome == undefined) {
        ...
        usuarioModel.cadastrar(nome, email, senha, telefone, documento, empresaId)
            .then(
                function (resultado) {
                    res.json(resultado);
                }
